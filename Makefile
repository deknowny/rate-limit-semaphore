...
# Reformat code style
format:
	$(poetry_exec) run black ralisem && \
	git add -u && \
	$(poetry_exec) run isort ralisem && \
	git add -u && \
	$(poetry_exec) run autoflake \
		--ignore-init-module-imports \
		--remove-unused-variables \
		--recursive \
		--in-place ralisem tests && \
	git add -u

# Run tests locally
test:
	$(poetry_exec) run pytest tests --cov=ralisem --cov-report=html

# Tests command for CI with .coveragerc report
test-ci:
	$(poetry_exec) run coverage run --source=ralisem -m pytest tests
